...
void on_start_clicked(void){
	puts("Start search");
	puts(PrefixPatern);
	if(regexp) puts("regexp true");
	if(highmode) puts("highmode true");
	char command[sizeof(PrefixPatern)+COMANDBUF];
	sprintf(command, "./%s %s %s %s", PROGRAMNAME, PrefixPatern, regexp?"-r":"", highmode?"-H":"" );
	system(command);
...
void on_ipv6_pater_changed(GtkWidget* data,gpointer p){
	...
	//GtkEntry * ipv6_label=GTK_ENTRY(gtk_builder_get_object(builder, "Ipv6Text"));
...
int main(int argc, char *argv[])
{
   ...
   // bzero(PrefixPatern, sizeof(PrefixPatern));
    ...
    //HighHeadSwitch = GTK_SWITCH(gtk_builder_get_object(builder, "regexpswitch"));
